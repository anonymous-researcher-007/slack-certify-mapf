#!/usr/bin/env python3
"""Validate the interval-aware + acyclicity-constrained wait-insertion ILP.

For each instance (n in {4,8,12,16,20}, seeds 1-5, delta=2) this:
...
warehouse fallback (real LaCAM* nominal plans). Logic is identical to the real
warehouse-10-20-10-2-1 / random-32-32-20 path; swap delta=2.
"""
...
def main() -> None:
    ...
    print("synthetic congested warehouse (MovingAI host blocked); real LaCAM* nominal plans")
    print("=" * 104)
    print(f"{'instance':12} {'status':>16} {'time_s':>9} {'obj':>5} "
    ...
    for n in N_AGENTS:
        ...
        for seed in SEEDS:
            ...
            try:
                ...
                print(f"n{n}/s{seed:<3} LaCAM* failed: {exc}")
                continue
            label = f"n{n}/s{seed}"
            ...
            print(f"{label:12} {status:>16} {rt:>9.2f} {str(obj):>5} "
